...
// League tracks Solo/Duo and Flex as two independent ladders. Per product
// decision, the cross-game Rank Score always takes whichever queue scores
// higher rather than defaulting to Solo/Duo — same principle already used
// for Overwatch's per-role best-of (getOverwatchScore below). A strong Flex
// rank shouldn't be invisible to the overall score just because Solo/Duo is
// lower or unplayed.
export function getLeagueBestEntry(rankData) {
    if (!Array.isArray(rankData)) return null
    const solo = rankData.find(e => e.queueType === "RANKED_SOLO_5x5") ?? null
    const flex = rankData.find(e => e.queueType === "RANKED_FLEX_SR") ?? null
    const soloScore = solo ? getLeagueScore(solo.tier, solo.rank) : null
    const flexScore = flex ? getLeagueScore(flex.tier, flex.rank) : null
    if (soloScore == null && flexScore == null) return null
    if (flexScore != null && (soloScore == null || flexScore > soloScore)) {
        return { entry: flex, score: flexScore, queue: "flex" }
    }
    return { entry: solo, score: soloScore, queue: "solo" }
}
...
// Marvel Rivals' ranked ladder (Bronze -> One Above All), each with III/II/I
// divisions except the top three tiers which are undivided. Percentiles are
// rough community estimates (marvelrivalsapi.com doesn't publish a
// distribution) — same placeholder caveat as the other tables in this file.
const MARVEL_RIVALS_TIER_TOP_PERCENT = {
    "ONE ABOVE ALL": 0.05,
    ETERNITY: 0.5,
    CELESTIAL: 3,
    GRANDMASTER: 10,
...
const MARVEL_RIVALS_TIER_ORDER = ["BRONZE", "SILVER", "GOLD", "PLATINUM", "DIAMOND", "GRANDMASTER", "CELESTIAL", "ETERNITY", "ONE ABOVE ALL"]
const MARVEL_RIVALS_DIVISIONS = ["III", "II", "I"]

// Rank string comes as e.g. "Gold III" or "One Above All" (top 3 tiers have
// no division). Splits off a trailing roman-numeral division if present.
export function getMarvelRivalsScore(rankName) {
    if (!rankName) return null
    const parts = rankName.trim().toUpperCase().split(' ')
    const lastPart = parts[parts.length - 1]
    const hasDivision = MARVEL_RIVALS_DIVISIONS.includes(lastPart)
    const tierKey = hasDivision ? parts.slice(0, -1).join(' ') : parts.join(' ')
    const topPercent = MARVEL_RIVALS_TIER_TOP_PERCENT[tierKey]
    if (topPercent === undefined) return null

    const tierIndex = MARVEL_RIVALS_TIER_ORDER.indexOf(tierKey)
    if (tierIndex <= 0 || !hasDivision) {
        return percentileToScore(topPercent)
    }

    const tierBelowPercent = MARVEL_RIVALS_TIER_TOP_PERCENT[MARVEL_RIVALS_TIER_ORDER[tierIndex - 1]]
    const divisionIndex = MARVEL_RIVALS_DIVISIONS.indexOf(lastPart) // III=0 .. I=2
    const progress = (divisionIndex + 1) / (MARVEL_RIVALS_DIVISIONS.length + 1)
    const interpolated = tierBelowPercent - (tierBelowPercent - topPercent) * progress
    return percentileToScore(interpolated)
}

// Per the game's own wiki (confirmed by the user 2026-07-04): each of the 7
// banded tiers (Bronze through Celestial) has 3 divisions worth 100 rank
// points each, so the raw rank_score axis runs 0-2099 across Bronze III up
// to Celestial I (7 tiers x 300 pts). Eternity and One Above All have no
// divisions and climb unbounded past that — consistent with every live
// rank_score we've observed for known-Eternity+/OAA players landing well
// above 2100 (e.g. a confirmed "One Above All" account sat at ~5200-5900
// across many seasons). Can't tell Eternity apart from One Above All by
// score alone (OAA is just "top 500 at season end", not a score cutoff),
// so anything >= 2100 is labeled "Eternity" — an intentional approximation,
// not a bug.
const MARVEL_RIVALS_BANDED_TIERS = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "Grandmaster", "Celestial"]
const MARVEL_RIVALS_POINTS_PER_DIVISION = 100
const MARVEL_RIVALS_DIVISION_LABELS = ["III", "II", "I"]

export function estimateMarvelRivalsRankFromScore(score) {
    if (score == null) return null
    const clamped = Math.max(0, Math.round(score))
    const bandedMax = MARVEL_RIVALS_BANDED_TIERS.length * MARVEL_RIVALS_DIVISION_LABELS.length * MARVEL_RIVALS_POINTS_PER_DIVISION
    if (clamped >= bandedMax) return "Eternity"

    const pointsPerTier = MARVEL_RIVALS_DIVISION_LABELS.length * MARVEL_RIVALS_POINTS_PER_DIVISION
    const tierIndex = Math.floor(clamped / pointsPerTier)
    const remainder = clamped % pointsPerTier
    const divisionIndex = Math.floor(remainder / MARVEL_RIVALS_POINTS_PER_DIVISION)
    return `${MARVEL_RIVALS_BANDED_TIERS[tierIndex]} ${MARVEL_RIVALS_DIVISION_LABELS[divisionIndex]}`
}
