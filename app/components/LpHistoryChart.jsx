...
function buildEstimatedPoints(matchHistory, currentLeaguePoints) {
    const rankedMatches = matchHistory
        .filter(m => m.queueId === RANKED_SOLO_QUEUE_ID)
        .sort((a, b) => b.gameEndTimestamp - a.gameEndTimestamp)

    let runningLp = currentLeaguePoints
    const points = [{ timestamp: Date.now(), lp: runningLp, delta: null, champion: null, win: null, isEstimated: true }]

    for (const match of rankedMatches) {
        const delta = match.win ? ESTIMATED_LP_GAIN : -ESTIMATED_LP_LOSS
        const lpBefore = Math.max(0, Math.round(runningLp - delta))
        points.push({ timestamp: match.gameEndTimestamp, lp: lpBefore, delta, champion: match.champion, win: match.win, isEstimated: true })
        runningLp = lpBefore
    }
...
export default function LpHistoryChart({ accountId, matchHistory, currentLeaguePoints, accentColor = "#b16cff", ddragonVersion = null }) {
                ...
                {tooltip && (
                    ...
                    >
                        ...
                        <div className="whitespace-nowrap">
                            ...
                            {tooltip.point.champion && (
                                <p className="text-text-secondary text-[10px]">{tooltip.point.champion} · {tooltip.point.win ? "Win" : "Loss"}</p>
                            )}
                        </div>
                    </div>
                )}
